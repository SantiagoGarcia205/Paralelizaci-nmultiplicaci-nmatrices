...
def sequential_matrix_multiplication(A, B): 
    ...
    mid = rows_A // 2
    mid2= rows_A // 4
    t1 = threading.Thread(target=worker, args=(0, mid2))
    t2 = threading.Thread(target=worker, args=(mid2, mid))
    t3= threading.Thread(target=worker, args=(mid, mid2+mid))
    t4= threading.Thread(target=worker, args=(mid2+mid, rows_A))
