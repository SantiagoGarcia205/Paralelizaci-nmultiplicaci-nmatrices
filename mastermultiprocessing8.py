...
def sequential_matrix_multiplication(A, B): 
    ...
    p6 =multiprocessing.Process(target=worker, args=(cols_B,cols_A,A,B,mid*5,mid*6,q))
    p7 =multiprocessing.Process(target=worker, args=(cols_B,cols_A,A,B,mid*6,mid*7,q))
    p8 =multiprocessing.Process(target=worker, args=(cols_B,cols_A,A,B,mid*7,rows_A,q))
    ...
    for i, fila in bloque6:
        C[i] = fila
    for i, fila in bloque7:
        C[i] = fila
    for i, fila in bloque8:
        C[i] = fila
